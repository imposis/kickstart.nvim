return {
  ...
  config = function()
    ...
    map('n', '<leader><leader>', '<Cmd>call jukit#send#line()<CR>', vim.tbl_extend('force', opts, { desc = 'Send line to REPL' }))
    map('n', '<leader>jos', '<Cmd>call jukit#splits#output()<CR>', vim.tbl_extend('force', opts, { desc = 'Split output' }))
    map('v', '<CR>', '<Esc><Cmd>call jukit#send#selection()<CR>', vim.tbl_extend('force', opts, { desc = 'Send selection to REPL' }))
